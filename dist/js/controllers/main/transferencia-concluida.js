...
(async()=>{
    ...
    const valorTransf = usuario.transacoes.find((elemento) => 
        elemento.periodo.dia == Horario.criaMesDia() ? elemento.periodo: 0
    )

    const posicaoUltimaTransferencia = valorTransf.periodo.valores.length - 1

    let valor1 = (parseFloat(valorTransf.periodo.valores[posicaoUltimaTransferencia])-9.30).toFixed(2)
    let valor2 = usuario.saldo.toFixed(2)

    frase.innerHTML = `<p>Você transferiu <strong>R$${valor1.replace('.', ',')}</strong>. Seu saldo agora é de <strong>R$${valor2.replace('.', ',')}</strong></p>`
