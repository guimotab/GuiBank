...
export class OperacaoRealizada {
    ...
    static criaTexto(tipoExtrato, valorExtrato, usuario) {
    ...
    static opcaoDeposito(titulo, frase, valorExtrato, saldoUsuario, usuario) {
        if (parseFloat(valorExtrato) > 8000) {
            ...
            frase.innerHTML = `<p>Você depositou <strong>R$${valorExtrato.replace('.', ',')}</strong> 
                ...
                <p>Você recebeu <strong>R$${(valorExtrato * 0.001).toFixed(2).replace('.', ',')}</strong>. 
                Seu saldo agora é de <strong>R$${saldoUsuario.replace('.', ',')}</strong></p>`
            valorExtrato = (valorExtrato * 0.001).toFixed(2)
    ...
    static opcaoSaque(titulo, frase, valorExtrato, saldoUsuario) {
    ...
    static opcaoTransferencia(titulo, frase, valorExtrato, saldoUsuario) {
